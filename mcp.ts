  ...
  return {
    tools: [
      ...
      },
      {
        ...
        {
          name: 'smart_search',
          description: 'Intelligently search for assignments, homework, files, or content using natural language. Can find homework assignments and their files automatically. Returns compact citations by default for efficiency.',
          inputSchema: {
            type: 'object',
            properties: {
              query: {
              ...
              returnMode: {
                ...
                description: 'Output format: "refs" for compact citations (default), "answer" for AI-generated response, "full" for complete details',
                default: 'refs'
              },
              useSmallModel: {
                type: 'boolean',
                description: 'Enable AI-powered intent classification and result reranking for better relevance (default: true)',
                default: true
              }
            },
            required: ['query'],
...
async function handleMultiCourseAssignmentSearch(
      ...
      if (options.useSmallModel) {
        try {
          ...
          if (contentSearch.success && 'citations' in contentSearch) {
            searchResults.push(...contentSearch.citations.map((citation: any) => ({
              ...citation,
              courseId: course.id,
              courseName: course.name
            })));
  ...
  if (queryLower.includes('due') && (queryLower.includes('today') || queryLower.includes('this week'))) {
  ...
  } else if (queryLower.includes('overdue') || queryLower.includes('missing')) {
    const today = new Date();
    filteredAssignments = allAssignments.filter((assignment: any) => {
      if (!assignment.dueDate) return false;
      const dueDate = new Date(assignment.dueDate);
      return dueDate < today && (!assignment.submittedAt || assignment.submittedAt === null);
    });
  } else if (queryLower.includes('not submitted') || queryLower.includes("haven't turned in")) {
    filteredAssignments = allAssignments.filter((assignment: any) => 
  ...
  // Limit results
  const limitedAssignments = filteredAssignments.slice(0, options.maxResults * 2);
  ...
  if (limitedAssignments.length > 0) {
    ...
    for (const assignment of limitedAssignments) {
      ...
      const submittedText = assignment.submittedAt ? '✅ Submitted' : '❌ Not submitted';
      const courseText = assignment.courseCode ? `${assignment.courseCode}` : assignment.courseName;
      
      markdown += `### ${assignment.name}\n`;
      markdown += `- **Course:** ${courseText}\n`;
      markdown += `- **Due:** ${dueText}\n`;
      markdown += `- **Status:** ${submittedText}\n`;
      if (assignment.points) markdown += `- **Points:** ${assignment.points}\n`;
      markdown += `\n`;
    }
  }
    ...
    try {
      ...
      case 'smart_search': {
        ...
        try {
          ...
          const courses = await listCourses({...getCanvasConfig(), enrollmentState: 'all'});
          
          // Filter for truly active/current courses (2025+ only)
          const currentYear = new Date().getFullYear();
          const activeCourses = courses.filter(c => {
            // Check enrollment state
            const hasActiveState = !c.enrollmentState || 
            ...
            for (const pattern of semesterYearPatterns) {
              ...
              for (const match of matches) {
                ...
                if (pattern.source.includes('spring|summer|fall')) {
                  ...
                  semester = match[1] === 'sp' ? 'spring' : match[1] === 'su' ? 'summer' : 'fall';
                  year = 2000 + parseInt(match[2]);
                } else if (pattern.source.includes('\\d{2}.*sp|su|fa')) {
                  // Year-semester: "25SP", "24FA"
                  year = 2000 + parseInt(match[1]);
                  semester = match[2] === 'sp' ? 'spring' : match[2] === 'su' ? 'summer' : 'fall';
                } else {
                  // Just year: "2024", "2025"
                ...
                if (year < currentYear) {
                  ...
                  if (courseSemesterOrder && courseSemesterOrder < currentSemesterOrder) {
          ...
          if (activeCourses.length === 0) {
            ...
            const courseDebug = courses.map(c => `${c.name} (${c.enrollmentState || 'no state'})`).join(', ');
            return {
              content: [
                {
                  type: "text",
          ...
          const assignmentKeywords = ['due', 'assignment', 'homework', 'submit', 'turn in', 'turned in', 'overdue', 'today', 'week', 'grade', 'missing'];
          const isAssignmentQuery = assignmentKeywords.some(keyword => queryLower.includes(keyword));
          ...
          for (const course of sortedCourses) {
            ...
            if (course.name.startsWith('Course ') && !queryLower.includes(course.id.toString())) {
              continue;
            }
            ...
            if (subjectNumberMatch) {
              ...
              if (subjectAliases[subject]) {
                subjectAliases[subject].forEach((alias: string) => {
                  ...
                  if (queryLower === alias || queryLower.includes(` ${alias} `) || queryLower.startsWith(`${alias} `) || queryLower.endsWith(` ${alias}`)) {
          ...
          if (isAssignmentQuery && !specificCourse) {
            return await handleMultiCourseAssignmentSearch(query, activeCourses, getCanvasConfig(), {
              maxResults,
              returnMode,
              useSmallModel
            });
          ...
          if ('citations' in searchResult) {
            ...
            if (citations.length > 0) {
              ...
              citations.forEach((citation, i) => {
                const icon = citation.type === 'file' ? '📁' : citation.type === 'page' ? '📄' : '🔗';
                const relevance = (citation.relevance * 100).toFixed(1) + '%';
                const canProcessInfo = citation.type === 'file' && citation.canProcess ? ' ✅' : citation.type === 'file' ? ' ❌' : '';
            ...
            if (searchResult.results.pages.length > 0) {
              ...
              searchResult.results.pages.slice(0, 5).forEach((page: any, i: number) => {
            ...
            if (searchResult.results.links.length > 0) {
              ...
              searchResult.results.links.slice(0, 5).forEach((link: any, i: number) => {
