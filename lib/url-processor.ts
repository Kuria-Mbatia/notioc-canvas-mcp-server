...
export interface CanvasURLInfo {
  type: 'file' | 'page' | 'assignment' | 'discussion' | 'module' | 'course' | 'unknown';
  courseId: string;
  resourceId?: string;
  resourceName?: string;
  url: string;
 ...
 */
export function extractFileIdsFromHTML(html: string, baseUrl: string): Array<{fileId: string, fileName: string, url: string}> {
  ...
  const canvasFileRegex = /class="instructure_file_link"[^>]*href="[^"]*\/files\/(\d+)[^"]*"[^>]*title="([^"]*)"[^>]*>/g;
  ...
  while ((match = apiRegex.exec(html)) !== null) {
    ...
    const fileName = nameMatch ? (nameMatch[1] || nameMatch[2] || `File ${fileId}`).trim() : `File ${fileId}`;
 ...
 */
export function extractLinksFromHTML(html: string): Array<{title: string, url: string, type: string}> {
  ...
  while ((match = externalRegex.exec(html)) !== null) {
    ...
    // Determine link type
    let type = 'external';
    if (url.includes('youtube.com') || url.includes('vimeo.com') || url.includes('mediaspace')) {
      type = 'video';
    } else if (url.includes('.pdf') || url.includes('.doc') || url.includes('.ppt')) {
      type = 'document';
    }
 ...
 */
export async function validateCanvasURL(url: string, accessToken: string): Promise<boolean> {
  try {
