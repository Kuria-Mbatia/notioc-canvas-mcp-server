...
interface McpContext {
  findCourseId: (courseName: string, canvasBaseUrl: string, accessToken: string) => Promise<string | undefined>;
}
...
export function createMCP(options: any): McpInstance {
  const findCourseId = async (courseName: string, canvasBaseUrl: string, accessToken: string): Promise<string | undefined> => {
