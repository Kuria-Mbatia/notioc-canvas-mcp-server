...
const intentCache = new Map<
  string,
  { result: IntentClassification; timestamp: number }
>();
const rerankCache = new Map<
  string,
  { result: RerankResult[]; timestamp: number }
>();
...
async function callOpenRouter(
  ...
  for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
    try {
      ...
      logger.debug(
        `[Small Model] Attempt ${attempt + 1} failed: ${error instanceof Error ? error.message : String(error)}`,
      );

      if (attempt === config.maxRetries) {
        throw new Error(
          `OpenRouter API failed after ${config.maxRetries + 1} attempts: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
...
Respond only with valid JSON.`;
    ...
    logger.info(
      `[Small Model] Intent classified: ${Object.entries(intent)
        .filter(([k, v]) => k !== "confidence" && k !== "reasoning" && v)
        .map(([k]) => k)
        .join(", ")} (confidence: ${intent.confidence})`,
    );

    return intent;
  } catch (error) {
    logger.warn(
      `[Small Model] Intent classification failed: ${error instanceof Error ? error.message : String(error)}`,
    );
...
export async function rerankCandidates(
  ...
  if (cached && Date.now() - cached.timestamp < config.cacheTTL) {
    logger.debug(
      `[Small Model] Rerank cache hit: ${cacheKey.substring(0, 50)}...`,
    );
  ...
  try {
    logger.debug(
      `[Small Model] Reranking ${candidates.length} candidates for: "${query}"`,
    );
...
Respond only with valid JSON array.`;
    ...
    logger.info(
      `[Small Model] Reranked to top ${validResults.length} candidates (avg score: ${(validResults.reduce((sum, r) => sum + r.score, 0) / validResults.length).toFixed(2)})`,
    );

    return validResults;
  } catch (error) {
    logger.warn(
      `[Small Model] Reranking failed: ${error instanceof Error ? error.message : String(error)}`,
    );
 ...
 * Clear caches
 */
export function clearSmallModelCache(): {
  intentCleared: number;
  rerankCleared: number;
} {
  ...
  logger.info(
    `[Small Model] Cleared caches: ${intentCleared} intent + ${rerankCleared} rerank entries`,
  );
