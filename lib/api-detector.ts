...
export async function testCourseAPIAvailability(
  ...
  if (useCache) {
    ...
    if (cached && cached.apiAvailability) {
      logger.debug(`[API Detector] Using cached API availability for course ${courseId}`);
  ...
  try {
    ...
    results.forEach((result, index) => {
      const endpointName = CANVAS_API_ENDPOINTS[index].name as keyof typeof availability.endpoints;
    ...
    if (useCache) {
      ...
      const updatedCache = {
        ...
        metadata: {
          ...
          hasRestrictedAPIs: Object.values(availability.endpoints).some(e => !e.available),
    ...
    logger.info(`[API Detector] API testing completed for course ${courseId} in ${totalTime}ms`);
    
    return buildTestResult(availability, totalTime);
    
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    logger.error(`[API Detector] Failed to test APIs for course ${courseId}: ${errorMsg}`);
    throw error;
  }
}
...
async function testSingleEndpoint(
  ...
  try {
    ...
    if (response.ok) {
      logger.debug(`[API Detector] ✅ ${endpoint.name}: Available (${response.status}, ${responseTime}ms)`);
      return {
        name: endpoint.name,
        path: endpoint.path,
        available: true,
      ...
      logger.debug(`[API Detector] ❌ ${endpoint.name}: Restricted (${response.status}, ${responseTime}ms) - ${errorMessage}`);
      return {
        name: endpoint.name,
        path: endpoint.path,
        available: false,
 ...
 */
function buildTestResult(availability: CourseAPIAvailability, totalTime: number): APITestResult {
 ...
 */
export function isAPIAvailable(testResult: APITestResult, apiType: string): boolean {
  const endpoint = testResult.availability.endpoints[apiType as keyof typeof testResult.availability.endpoints];
 ...
 */
export function getSuggestedFallbacks(testResult: APITestResult): Array<{api: string, fallback: string, reason: string}> {
