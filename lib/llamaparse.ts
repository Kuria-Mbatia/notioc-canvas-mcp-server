...
function validateInput(input: ParseInput, options: LlamaParseOptions): void {
  ...
  if (isAudio && input.buffer.length > audioLimit) {
    throw createError(
      "SIZE_EXCEEDED",
      `Audio file exceeds 20MB limit (${(input.buffer.length / 1024 / 1024).toFixed(1)}MB)`,
    );
  }
  ...
  if (input.buffer.length > maxBytes) {
    throw createError(
      "SIZE_EXCEEDED",
      `File exceeds ${Math.round(maxBytes / 1024 / 1024)}MB limit (${(input.buffer.length / 1024 / 1024).toFixed(1)}MB)`,
    );
  }
}
  ...
  while (Date.now() - startTime < timeoutMs) {
    try {
      ...
      if (status.status === "ERROR" || status.status === "FAILED") {
        throw createError(
          "API_ERROR",
          `Parse job failed: ${status.error || "Unknown error"}`,
          status,
        );
      }
...
async function getResult(
  ...
  const response = await fetch(
    `${baseUrl}/api/parsing/job/${jobId}/result/${options.resultFormat || "markdown"}`,
    {
      headers: {
        Authorization: `Bearer ${options.apiKey}`,
      },
...
export async function parseWithLlama(
  ...
  try {
    ...
    logger.debug(
      `[LlamaParse] Starting parse: ${input.filename} (${(input.buffer.length / 1024).toFixed(1)}KB)`,
    );
    ...
    logger.info(
      `[LlamaParse] Parse successful: ${input.filename} (${processingTime}ms)`,
    );
    ...
    if (error instanceof Error && (error as LlamaParseError).code) {
      logger.warn(
        `[LlamaParse] Parse failed: ${input.filename} - ${error.message} (${processingTime}ms)`,
      );
      throw error;
    }

    logger.error(
      `[LlamaParse] Unexpected error: ${input.filename} - ${error} (${processingTime}ms)`,
    );
