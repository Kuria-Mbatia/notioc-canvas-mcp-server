 ...
 */
export function compressToPreview(content: string, maxChars: number = DEFAULT_FILE_CACHE_CONFIG.previewMaxChars): string {
...
export function getCachedFileContent(
  ...
  if (age > config.ttl) {
    ...
    logger.debug(`[File Cache] Expired cache entry removed: ${cacheKey} (age: ${Math.round(age / 1000 / 60)} minutes)`);
    return null;
  }
  ...
  logger.debug(`[File Cache] Cache hit: ${cacheKey} (age: ${Math.round(age / 1000 / 60)} minutes)`);
  return cached;
}
...
export function setCachedFileContent(
  ...
  if (content.length > config.maxContentSize) {
    logger.warn(`[File Cache] Content too large to cache: ${cacheKey} (${Math.round(content.length / 1024 / 1024)}MB > ${Math.round(config.maxContentSize / 1024 / 1024)}MB)`);
    return;
  }
  ...
  logger.debug(`[File Cache] Cached content: ${cacheKey} (${Math.round(content.length / 1024)}KB, preview: ${Math.round(preview.length / 1024)}KB)`);
}
 ...
 * Clear file cache
 */
export function clearFileCache(fileId?: string): { cleared: number; message: string } {
  let cleared = 0;
 ...
 */
export function cleanupExpiredEntries(config: FileCacheConfig = DEFAULT_FILE_CACHE_CONFIG): number {
  const now = Date.now();
  const keysToDelete: string[] = [];
  ...
  if (keysToDelete.length > 0) {
    logger.info(`[File Cache] Cleaned up ${keysToDelete.length} expired entries`);
  }
