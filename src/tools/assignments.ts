...
function extractFileLinks(
  htmlContent: string,
  canvasBaseUrl: string,
): Array<{ id: string; url: string; text: string }> {
  ...
  patterns.forEach((regex, index) => {
    ...
    while ((match = regex.exec(htmlContent)) !== null) {
      if (index === 0) {
        ...
        const completeUrl = fullUrl.startsWith("http")
          ? fullUrl
...
export async function listAssignments(
  ...
  try {
    ...
    const assignments: AssignmentInfo[] = assignmentsData.map((assignment) => {
      ...
      const embeddedFileLinks = extractFileLinks(
...
export async function getAssignmentDetails(params: {
  ...
  AssignmentInfo & {
    allFiles: Array<{ id: string; name: string; url: string; source: string }>;
  }
> {
  let {
    canvasBaseUrl,
  ...
  if (assignmentId) {
    ...
    const foundAssignment = findBestMatch(assignmentName, assignments, [
      "name",
    ]);
    if (!foundAssignment) {
