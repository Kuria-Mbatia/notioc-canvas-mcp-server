...
export async function postDiscussionReply(
  ...
  try {
    ...
    logger.info(
      `Posting reply to discussion topic ${topicId} in course ${courseId}`,
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to post discussion reply: ${response.status} ${response.statusText} - ${errorText}`,
      );
    }
 ...
 */
export async function replyToDiscussionEntry(
  params: PostDiscussionReplyParams & { parentEntryId: string },
): Promise<DiscussionReplyResult> {
  let {
    canvasBaseUrl,
    accessToken,
  ...
  try {
    ...
    logger.info(
      `Posting threaded reply to entry ${parentEntryId} in topic ${topicId}, course ${courseId}`,
    );
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to post threaded reply: ${response.status} ${response.statusText} - ${errorText}`,
      );
    }
