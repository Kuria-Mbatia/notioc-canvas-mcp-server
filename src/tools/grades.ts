  ...
  try {
    ...
    if (courseId || courseName) {
      ...
      if (courseId) {
        ...
        targetCourse = findBestMatch(courseName, allCourses, [
          "name",
          "courseCode",
          "nickname",
        ]);
    ...
    for (const course of coursesToFetch) {
      logger.info(
        `Fetching grades for user ${userId} in course ${course.name} (ID: ${course.id})`,
      );
      ...
      if (userId === "self") {
        ...
        enrollments = await fetchAllPaginated<any>(
          ...
          `/api/v1/courses/${course.id}/enrollments`,
          {
            ...
            include: ["current_score", "final_score", "grades"],
            per_page: "100",
          },
        );
      } else {
      ...
      logger.info(
        `Found ${enrollments.length} enrollments for course ${course.id}`,
      );
      ...
      if (!userEnrollment) {
        logger.warn(
          `User ${userId} not enrolled in course ${course.name}. Skipping.`,
        );
        continue;
      }
      ...
      if (assignmentId || assignmentName) {
        ...
        if (assignmentId) {
          ...
          targetAssignment = findBestMatch(assignmentName, assignments, [
            "name",
          ]);
        }
        ...
        if (!targetAssignment) {
          throw new Error(
            `Could not find assignment matching "${assignmentName || assignmentId}" in course "${course.name}".`,
          );
        }
      ...
      try {
        ...
        if (userId === "self") {
          try {
            ...
            if (response.ok) {
              ...
              logger.info(
                `Fetched ${userSubmissions.length} submissions for course ${course.id} efficiently`,
              );
            } else {
              throw new Error(
                `Failed to fetch all submissions: ${response.status}`,
              );
            }
          } catch (bulkError: any) {
            logger.warn(
              `Bulk submission fetch failed for course ${course.id}, falling back to individual assignment fetch: ${bulkError.message}`,
            );
            ...
            for (const assignment of assignmentsToProcess.slice(0, 10)) {
              // Limit to first 10 assignments to avoid hanging
              try {
                ...
                const timeoutPromise = new Promise((_, reject) =>
                  setTimeout(
                    () => reject(new Error("Assignment fetch timeout")),
                    ASSIGNMENT_TIMEOUT,
                  ),
                );
                ...
                logger.warn(
                  `Could not fetch submission for assignment ${assignment.id}: ${assignmentError}`,
                );
        ...
        logger.warn(
          `Could not fetch submissions for course ${course.id}: ${submissionError}`,
        );
      }
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        logger.warn(
          `Grades not available for course ${courseId}: ${error.message}`,
        );
      ...
      if (
        ...
        throw new Error(
          `Access denied - insufficient permissions to view grades`,
        );
      }
...
export async function getGradebookCategories(
  params: GradebookCategoriesParams,
): Promise<GradebookCategoryInfo[]> {
  ...
  try {
    ...
    if (courseName && !courseId) {
      ...
      if (!matchedCourse) {
        throw new Error(
          `Could not find an active course with the name "${courseName}".`,
        );
      }
    ...
    const categories: GradebookCategoryInfo[] = assignmentGroups.map(
      (group: any) => ({
        name: group.name,
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        logger.warn(
          `Gradebook categories not available for course ${courseId}: ${error.message}`,
        );
        return []; // Return empty array instead of throwing
      }
      if (
      ...
      if (
        ...
        throw new Error(
          `Access denied - insufficient permissions to view gradebook for course ${courseId}`,
        );
      }
