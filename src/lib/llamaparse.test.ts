...
function mockFetch(
  responses: Array<{ url: RegExp; response: any; status?: number }>,
) {
...
describe("LlamaParse Client", () => {
  ...
  describe("Input validation", () => {
    test("should reject when API key is missing", async () => {
      ...
      await expect(parseWithLlama(validInput, options)).rejects.toMatchObject({
        code: "DISABLED",
    ...
    test("should reject unsupported file types", async () => {
      ...
      await expect(parseWithLlama(input, validOptions)).rejects.toMatchObject({
        code: "UNSUPPORTED",
    ...
    test("should reject files exceeding size limit", async () => {
      ...
      await expect(parseWithLlama(input, options)).rejects.toMatchObject({
        code: "SIZE_EXCEEDED",
      });
    });
  ...
  describe("API interaction", () => {
    ...
    test("should handle upload failures", async () => {
      ...
      await expect(
        parseWithLlama(validInput, validOptions),
