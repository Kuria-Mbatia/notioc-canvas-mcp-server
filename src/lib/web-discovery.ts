...
export async function discoverCourseContentViaWeb(
  ...
  try {
    ...
    if (opts.includeNavigation) {
      const navigation = await discoverCourseNavigation(
        courseId,
        canvasBaseUrl,
        accessToken,
      );
      if (navigation.pages.length > 0) {
        ...
        logger.info(
          `[Web Discovery] Found ${navigation.pages.length} pages via navigation`,
        );
      }
    }

    // Step 2: Try common course page patterns
    const commonPages = await discoverCommonPages(
      courseId,
      canvasBaseUrl,
      accessToken,
      opts,
    ...
    commonPages.forEach((page) => {
      if (
        !result.discoveredPages.some((existing) => existing.url === page.url)
      ) {
    ...
    logger.info(
      `[Web Discovery] Total discovered pages: ${result.discoveredPages.length}`,
    );
    ...
    if (opts.extractEmbeddedContent) {
      for (const page of result.discoveredPages) {
        if (page.accessible && page.url) {
          try {
            const contentResult = await extractContentFromPage(
              page,
              canvasBaseUrl,
              accessToken,
            );
            ...
            result.warnings.push(
              `Failed to extract content from ${page.name}: ${errorMsg}`,
            );
            logger.warn(
              `[Web Discovery] Content extraction failed for ${page.name}: ${errorMsg}`,
            );
          }
        }
      }
    ...
    result.success =
      result.discoveredPages.length > 0 || result.discoveredFiles.length > 0;

    logger.info(
      `[Web Discovery] Completed for course ${courseId}: ${result.timing.pagesDiscovered} pages, ${result.timing.filesExtracted} files, ${result.timing.linksExtracted} links in ${result.timing.totalTime}ms`,
    );
...
async function discoverCourseNavigation(
  ...
  try {
    logger.debug(
      `[Web Discovery] Discovering navigation for course ${courseId}`,
    );
...
async function discoverCommonPages(
  ...
  logger.debug(
    `[Web Discovery] Testing common page patterns for course ${courseId}`,
  );

  const discoveredPages: DiscoveredPage[] = [];
  const testPromises = COMMON_COURSE_PAGES.slice(0, options.maxPages).map(
    (pageName) =>
  ...
  try {
    ...
    results.forEach((result, index) => {
      if (result.status === "fulfilled" && result.value) {
        ...
        logger.debug(
          `[Web Discovery] ❌ Page not found: ${COMMON_COURSE_PAGES[index]}`,
        );
      }
    });
 ...
 */
async function isPageAccessible(
  url: string,
  accessToken: string,
): Promise<boolean> {
  try {
  ...
  try {
    ...
    try {
      ...
      if (apiResponse.ok) {
        ...
        html = pageData.body || "";
        logger.debug(
          `[Web Discovery] Got content via API for ${page.name} (${html.length} chars)`,
        );
      } else {
      ...
      logger.debug(
        `[Web Discovery] API failed for ${page.name}, trying web interface`,
      );
    ...
    for (const match of fileMatches) {
      ...
      try {
        ...
        if (fileResponse.ok) {
          ...
          actualFileName =
            fileData.filename || fileData.display_name || match.fileName;
          logger.debug(
            `[Web Discovery] Enhanced file ${match.fileId}: "${match.fileName}" → "${actualFileName}"`,
          );
        }
      } catch (error) {
        // Keep original name if API fails
        logger.debug(
          `[Web Discovery] Could not enhance file ${match.fileId}, keeping: "${match.fileName}"`,
        );
      }
    ...
    logger.debug(
      `[Web Discovery] Extracted ${files.length} files and ${links.length} links from ${page.name}`,
    );
...
export function searchDiscoveredContent(
  ...
  const totalMatches =
    matchingFiles.length + matchingPages.length + matchingLinks.length;
  const totalContent =
    discovery.discoveredFiles.length +
    discovery.discoveredPages.length +
    discovery.discoveredLinks.length;
