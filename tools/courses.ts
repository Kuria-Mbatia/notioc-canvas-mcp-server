...
export async function getCourseSyllabus(params: {
  canvasBaseUrl: string;
  accessToken: string;
  courseId: string;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch course syllabus: ${response.status} ${response.statusText}`,
      );
    }

    const courseData: { syllabus_body?: string; html_url: string } =
      await response.json();
    ...
    if (error instanceof Error) {
      throw new Error(
        `Failed to fetch course syllabus for course ${courseId}: ${error.message}`,
      );
    } else {
      throw new Error(
        `Failed to fetch course syllabus for course ${courseId}: Unknown error`,
      );
    }
  }
}
