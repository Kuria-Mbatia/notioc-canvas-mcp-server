...
import { parseCanvasURL, extractFileIdsFromHTML, extractLinksFromHTML, validateCanvasURL } from '../lib/url-processor.js';
 ...
 */
export async function processCanvasURL(params: ProcessCanvasURLParams): Promise<ProcessedURLResult> {
  ...
  try {
    ...
    logger.debug(`[URL Processor] Parsed URL: ${urlInfo.type} in course ${urlInfo.courseId}`);
    ...
    let embeddedFiles: Array<{fileId: string, fileName: string, url: string}> = [];
      ...
      case 'file':
        if (urlInfo.resourceId && params.processFiles) {
          try {
            ...
            method = 'api';
            logger.info(`[URL Processor] Processed file ${urlInfo.resourceId} via API`);
                     } catch (error) {
             const errorMsg = error instanceof Error ? error.message : String(error);
             logger.warn(`[URL Processor] File API failed, trying direct download: ${errorMsg}`);
      ...
      case 'page':
        if (urlInfo.resourceId) {
          try {
            ...
            method = 'api';
            logger.info(`[URL Processor] Processed page ${urlInfo.resourceId} via API`);
                     } catch (error) {
             const errorMsg = error instanceof Error ? error.message : String(error);
             logger.warn(`[URL Processor] Page API failed, trying web interface: ${errorMsg}`);
            ...
            try {
              ...
              if (response.ok) {
                ...
                logger.info(`[URL Processor] Processed page ${urlInfo.resourceId} via web interface`);
              } else {
                throw new Error(`Web interface failed: ${response.status}`);
              }
                         } catch (webError) {
               const webErrorMsg = webError instanceof Error ? webError.message : String(webError);
      ...
      default:
        ...
        try {
          ...
          if (response.ok) {
            ...
            content = html;
            method = 'web_interface';
            logger.info(`[URL Processor] Processed ${urlInfo.type} via web interface`);
          } else {
    ...
    if (params.extractEmbedded && content && method === 'web_interface') {
      try {
        ...
        logger.info(`[URL Processor] Extracted ${embeddedFiles.length} files and ${embeddedLinks.length} links`);
             } catch (error) {
         const errorMsg = error instanceof Error ? error.message : String(error);
         logger.warn(`[URL Processor] Failed to extract embedded content: ${errorMsg}`);
       }
    }
 ...
 */
export async function extractFileInfoFromURL(params: ProcessCanvasURLParams): Promise<{fileId: string, fileName?: string, accessible: boolean}[]> {
  try {
    const urlInfo = parseCanvasURL(params.url);
