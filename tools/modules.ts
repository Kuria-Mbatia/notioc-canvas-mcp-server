...
export async function listModules(
  ...
  try {
    ...
    logger.info(
      `Listing modules for course ${courseId}${searchTerm ? ` matching "${searchTerm}"` : ""}`,
    );
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        logger.warn(
          `Modules not available for course ${courseId}: ${error.message}`,
        );
      ...
      if (
        ...
        throw new Error(
          `Access denied - insufficient permissions to view modules for course ${courseId}`,
        );
      }
...
export async function getModuleItems(
  params: ModuleItemsParams,
): Promise<ModuleItemInfo[]> {
  let {
    canvasBaseUrl,
    accessToken,
    courseId,
  ...
  try {
    ...
    logger.info(
      `Getting items for module ${moduleId} in course ${courseId}${searchTerm ? ` matching "${searchTerm}"` : ""}`,
    );
...
export async function getModuleDetails(params: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to get module details: ${response.status} ${response.statusText} - ${errorText}`,
      );
    }
