 ...
 */
export async function getQuizSubmissionContent(params: QuizSubmissionContentParams): Promise<QuizSubmissionContent> {
  let { 
    canvasBaseUrl, 
    accessToken, 
    courseId, 
  ...
  try {
    ...
    if (!courseId || !quizId) {
      throw new Error('Must provide either submissionId or (courseId/courseName + quizId/quizName)');
    }
    ...
    if (!submissionsResponse.ok) {
      throw new Error(`Failed to fetch quiz submissions: ${submissionsResponse.status} - ${submissionsResponse.statusText}`);
    }

    const submissionsData = await submissionsResponse.json();
    
    logger.info(`Raw submissions response type: ${typeof submissionsData}, isArray: ${Array.isArray(submissionsData)}`);
    ...
    let submissions = [];
    if (submissionsData.quiz_submissions && Array.isArray(submissionsData.quiz_submissions)) {
    ...
    const completedSubmissions = submissions.filter((sub: any) => sub.workflow_state === 'complete');
    logger.info(`Found ${completedSubmissions.length} completed submissions`);
    
    const userSubmission = completedSubmissions.sort((a: any, b: any) => b.attempt - a.attempt)[0];

    if (!userSubmission) {
      // Try to get quiz assignment submission as fallback - this often works better
      logger.info('No completed submission found via quiz submissions endpoint, trying assignment approach');
      ...
      try {
        ...
        if (assignmentResponse.ok) {
          ...
          const quizAssignment = assignments.find((a: any) => a.quiz_id === parseInt(quizId || '0'));
          
          if (quizAssignment && quizAssignment.submission && quizAssignment.submission.workflow_state === 'graded') {
            logger.info(`Found quiz assignment ${quizAssignment.id} with submission ${quizAssignment.submission.id}`);
      ...
      throw new Error('No completed quiz submissions found for this quiz. Make sure you have taken and submitted the quiz.');
    }

    logger.info(`Found completed submission ${userSubmission.id} for quiz ${quizId}`);
    ...
    if (error instanceof Error) {
      throw new Error(`Failed to get quiz submission content: ${error.message}`);
    } else {
...
async function getQuizSubmissionContentBySubmissionId(
  ...
  logger.info(`Accessing quiz submission content for submission ${submissionId}`);
  ...
  try {
    ...
    if (!questionsResponse.ok) {
      if (questionsResponse.status === 403) {
        throw new Error('Access denied: This quiz submission may not be available for review, or the quiz may not be completed yet.');
      }
      throw new Error(`Failed to fetch quiz questions: ${questionsResponse.status}`);
    }

    questionsData = await questionsResponse.json();
    logger.info(`Successfully got quiz questions for submission ${submissionId}`);
    
  } catch (error) {
    throw new Error(`Failed to fetch quiz questions: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
  ...
  if (!submissionMetadata) {
    try {
      ...
      if (submissionResponse.ok) {
        ...
        logger.info(`Could not get submission metadata (${submissionResponse.status}), using questions data only`);
      }
    } catch (error) {
  ...
  const questions: QuizQuestionContent[] = questionsData.quiz_submission_questions?.map((q: any) => {
    ...
    if (q.incorrect_comments) question.incorrectComments = q.incorrect_comments;
  ...
  const totalPossible = questions.reduce((sum, q) => sum + (q.pointsPossible || 0), 0);
  ...
  const submissionPossible = submission.quiz?.points_possible || submission.points_possible || totalPossible || 20; // Default to 20 if unknown
    ...
    questions.forEach(q => {
      if (q.pointsPossible === 0) {
        ...
        if (q.correct !== undefined) {
          ...
          q.userScore = (submissionScore / submissionPossible) * estimatedPointsPerQuestion;
        }
      }
    });
  }
  ...
  logger.info(`Successfully retrieved ${questions.length} questions for quiz "${quizSubmissionContent.quizTitle}"`);
...
export async function getUserQuizSubmissionHistory(params: {
  ...
  courseName?: string;
  quizId: string;
}): Promise<Array<{ submissionId: string; attempt: number; score: number; submittedAt: string; }>> {
  ...
  if (!submissionsResponse.ok) {
    throw new Error(`Failed to fetch quiz submissions: ${submissionsResponse.status}`);
  }
