 ...
 */
export async function calculateCourseAnalytics(params: CourseAnalyticsParams): Promise<CourseAnalyticsInfo> {
  ...
  try {
    ...
    const [gradeData, categories, assignments] = await Promise.all([
      ...
      fetchAllPaginated<any>(canvasBaseUrl, accessToken, `/api/v1/courses/${courseId}/assignments`, { per_page: '100' })
    ]);
    ...
    const categoryBreakdown = categories.map(category => {
      const categoryAssignments = assignments.filter((a: any) => 
        a.assignment_group_id && String(a.assignment_group_id) === String(category.name) // This needs fixing
      );

      const completedAssignments = courseGrades.assignments?.filter((a: any) => 
        categoryAssignments.some((ca: any) => String(ca.id) === a.assignmentId) && a.score !== null
      ) || [];

      const pointsEarned = completedAssignments.reduce((sum: number, a: any) => sum + (a.score || 0), 0);
      const pointsPossible = completedAssignments.reduce((sum: number, a: any) => sum + a.pointsPossible, 0);

      return {
        categoryName: category.name,
        weight: category.weight,
        currentScore: pointsPossible > 0 ? (pointsEarned / pointsPossible) * 100 : null,
        pointsEarned,
        pointsPossible,
    ...
    const completedAssignments = courseGrades.assignments?.filter((a: any) => a.score !== null) || [];
    ...
    const averageScore = completedAssignments.length > 0 
      ? completedAssignments.reduce((sum: number, a: any) => {
          const percentage = a.pointsPossible > 0 ? (a.score / a.pointsPossible) * 100 : 0;
          return sum + percentage;
        }, 0) / completedAssignments.length 
      : null;

    // Calculate projected grade (assuming current performance continues)
    const totalPointsPossible = assignments.reduce((sum: any, a: any) => sum + (a.points_possible || 0), 0);
    const currentPointsEarned = completedAssignments.reduce((sum: number, a: any) => sum + (a.score || 0), 0);
    const remainingPoints = totalPointsPossible - completedAssignments.reduce((sum: number, a: any) => sum + a.pointsPossible, 0);
    ...
    const projectedPercentage = totalPointsPossible > 0 ? (projectedPointsEarned / totalPointsPossible) * 100 : null;
    ...
    const result: CourseAnalyticsInfo = {
      ...
      currentGrade: {
        ...
        pointsPossible: completedAssignments.reduce((sum: number, a: any) => sum + a.pointsPossible, 0)
      },
      projectedGrade: {
        percentage: projectedPercentage,
        letterGrade: projectedPercentage ? convertToLetterGrade(projectedPercentage) : null,
 ...
 */
export async function generateWhatIfScenarios(params: WhatIfScenarioParams): Promise<WhatIfScenarioInfo> {
  let { canvasBaseUrl, accessToken, courseId, courseName, targetGrade, targetLetterGrade, userId = 'self' } = params;
  ...
  if (!targetGrade) {
    throw new Error('Either targetGrade (percentage) or targetLetterGrade must be provided');
  }

  try {
    // Get current analytics
    const analytics = await calculateCourseAnalytics({ canvasBaseUrl, accessToken, courseId, courseName, userId });
    ...
    const remainingPointsPossible = totalPointsPossible - analytics.currentGrade.pointsPossible;

    // Calculate what's needed
    const targetPoints = (targetGrade / 100) * totalPointsPossible;
    const pointsNeeded = targetPoints - currentPointsEarned;
    const requiredAverage = remainingPointsPossible > 0 ? (pointsNeeded / remainingPointsPossible) * 100 : null;
    ...
    if (requiredAverage !== null) {
      ...
      if (requiredAverage > 90) {
        ...
        const lowerRequiredAverage = (lowerPointsNeeded / remainingPointsPossible) * 100;

        scenarios.push({
          description: `Alternative: Target ${lowerTarget}% grade instead`,
          requiredScore: lowerRequiredAverage,
          difficulty: lowerRequiredAverage <= 90 ? 'Moderate' as const : 'Challenging' as const,
    ...
    if (isAchievable) {
      if (requiredAverage! <= 85) {
        recommendations.push("Your target grade is very achievable with consistent effort.");
        recommendations.push("Focus on understanding concepts rather than just completing assignments.");
      } else if (requiredAverage! <= 95) {
        recommendations.push("Your target grade is achievable with strong performance.");
        recommendations.push("Consider forming study groups or attending office hours.");
        recommendations.push("Review assignment rubrics carefully to maximize points.");
      } else {
        recommendations.push("Your target grade requires near-perfect performance.");
        recommendations.push("Prioritize high-point value assignments.");
        recommendations.push("Seek help from instructors and tutors immediately.");
        recommendations.push("Consider extra credit opportunities if available.");
      }
    } else {
      recommendations.push("Unfortunately, this target grade may not be mathematically achievable.");
      recommendations.push("Consider adjusting your target or discussing options with your instructor.");
      recommendations.push("Focus on learning and doing your best on remaining assignments.");
    }
    ...
    if (highValueAssignments.length > 0) {
      recommendations.push(`Focus especially on these high-value upcoming assignments: ${highValueAssignments.map(a => a.name).join(', ')}`);
    }
 ...
 */
export async function getGradeTrends(params: GradeTrendsParams): Promise<GradeTrendsInfo> {
  let { canvasBaseUrl, accessToken, courseId, courseName, userId = 'self', daysBack = 90 } = params;
  ...
  try {
    ...
    const recentAssignments = courseGrades.assignments
      .filter((a: any) => a.gradedAt && new Date(a.gradedAt) > cutoffDate && a.score !== null)
      .sort((a: any, b: any) => new Date(a.gradedAt).getTime() - new Date(b.gradedAt).getTime());
    ...
    if (timelineData.length >= 3) {
      ...
      confidence = timelineData.length >= 6 ? 'High' : timelineData.length >= 4 ? 'Medium' : 'Low';
    }

    // Generate insights
    const insights = [];
    if (overallTrend === 'Improving') {
      insights.push(`Your grades are trending upward by ${trendPercentage!.toFixed(1)} percentage points.`);
      insights.push("Keep up the good work and maintain your current study habits.");
    } else if (overallTrend === 'Declining') {
      insights.push(`Your grades are trending downward by ${Math.abs(trendPercentage!).toFixed(1)} percentage points.`);
      insights.push("Consider reviewing your study strategies and seeking help if needed.");
    } else if (overallTrend === 'Stable') {
      insights.push("Your grades are consistent, which shows reliable performance.");
    }

    if (timelineData.length > 0) {
      const avgScore = timelineData.reduce((sum, a) => sum + a.percentage, 0) / timelineData.length;
      insights.push(`Your average score over the last ${daysBack} days is ${avgScore.toFixed(1)}%.`);
    }
...
function convertLetterGradeToPercentage(letterGrade: string): number {
  ...
  switch (grade) {
    ...
    case 'D-': return 62;
    case 'F': return 50;
    default: return 85; // Default to B
  }
}
